...
function drawTopo() {
    ...
    // Grid overlay (faint)
    topoCtx.strokeStyle = 'rgba(70,102,0,0.04)';
    topoCtx.lineWidth = 1;
...
// Restart pipeline when a new file finishes queueing
const origQueueMini = renderQueueMini;
...
// ================================================================
//  16. BACKGROUND LOG SIMULATION
// ================================================================
const OPS  = ['extract_pdf','chunk_text','gemini_req','save_db','vectorize','index_doc','cleanup_tmp'];
const FLOG = ['Neuro_Ch1.pdf','Quantum.docx','BladeRunner.pdf','AI_Ethics.pdf','DataSci.docx'];
function simulateBackend() {
    if (Math.random() > 0.38) {
        ...
        if (r>0.88) { type='warn'; status='<span class="log-warn">DELAY</span>'; }
        if (r>0.96) { type='err';  status='<span class="log-err">FAIL</span>'; }
        asostAddLog(`<span class="keyword">${op}</span> <span class="punct">→</span> <span class="string">"${f}"</span> <span class="punct">[</span>${status}<span class="punct">]</span>`, type);
    }
    setTimeout(simulateBackend, 900+Math.random()*2200);
}
...
// ================================================================
[
    {m:'asost-daemon v3.1.0 booting...',                             t:'sys',  d:200},
    {m:'loading kernel modules [<span class="log-sys">OK</span>]',  t:'sys',  d:650},
    {m:'sqlite3 db connected [<span class="log-sys">OK</span>]',    t:'info', d:1050},
    {m:'worker pool initialized: 12 threads',                        t:'info', d:1400},
    {m:'gemini-2.5-pro endpoint verified [<span class="log-sys">OK</span>]', t:'sys', d:1900},
    {m:'<span class="log-warn">WARN:</span> KEY-B approaching TPM limit: 58K/60K', t:'warn', d:2400},
    {m:'node topology graph: all nodes UP [<span class="log-sys">OK</span>]', t:'sys', d:2900},
    {m:'pipeline engine ready [<span class="log-sys">STANDBY</span>]', t:'sys', d:3400},
    {m:'listening on port 8080 [<span class="log-sys">READY</span>]', t:'sys', d:3900},
].forEach(({m,t,d}) => setTimeout(()=>asostAddLog(m,t), d));

setTimeout(simulateBackend, 4500);
setTimeout(()=>{renderQueueMini(); updateApiStats();}, 400);
...
initMatrix();
function drawMatrix() {
    matCtx.fillStyle='rgba(5,5,5,0.052)';
    matCtx.fillRect(0,0,matCanvas.width,matCanvas.height);
    matCtx.fillStyle='rgba(70,102,0,0.26)';
